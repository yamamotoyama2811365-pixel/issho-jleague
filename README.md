...
### 2026-09-17 試合・リーグナビゲーション修正

- 日程取得は公式の1試合カード内でホーム・アウェイ・時刻・会場・チケットを照合。ストリーミングHTMLの分割要素を結合し、別試合への取り違えを防止する。旧取り込みデータは再検証されるまで表示しない。
- トップはJ1/J2/J3の次節を全件表示。順位はTOP 5と各リーグの全順位をタブで切り替える。
- クラブ一覧はリーグ別・順位順。クラブ名/会場検索、次戦、日程、選手、ホーム会場への導線を追加。
- 静的生成前に軽量な日程同期を実行し、毎日06:20/22:20 JSTに生成。公開前に試合重複と各リーグ20クラブの順位・一覧を検査する。
- 回帰確認: `python -m unittest discover -s tests`。
